...
const TravelAssistantPanel = ({ isOpen, onClose }) => {
  ...
  const handleSendMessage = async (messageText = inputMessage) => {
    ...
    if (typeof messageText === 'string') {
      ...
      textContent = messageText.message || messageText.text || messageText.content || JSON.stringify(messageText);
    } else {
      textContent = String(messageText || '');
    }
    
    if (textContent.trim() !== '') {
      trackEvent('user_message_sent', { language: 'en', query: textContent });
      const newMessage = { id: Date.now(), text: textContent, sender: 'user', lang: 'en', feedback: null, timestamp: new Date() };
      setMessages(prevMessages => [...prevMessages, newMessage]);
      setInputMessage('');
      setIsContentSectionVisible(false);
      setIsTyping(true);

      const updatedHistory = [...chatHistoryForApi, { role: 'USER', message: textContent }];
      setChatHistoryForApi(updatedHistory);

      setTimeout(async () => {
        const lowerCaseMessage = textContent.toLowerCase();
        let aiResponse = "I'm sorry, I couldn't understand your request.";
        let matched = false;
        let responseType = 'rule_based';
        if (lowerCaseMessage.includes("what is stayfinder") || lowerCaseMessage.includes("about stayfinder") || lowerCaseMessage.includes("what is stay finder") || lowerCaseMessage.includes("about stay finder") || lowerCaseMessage.includes("tell me about stay finder") || lowerCaseMessage.includes("how does stay finder work")) {
          ...
          matched = true;
        } else if (lowerCaseMessage.includes("safe") || lowerCaseMessage.includes("safety") || lowerCaseMessage.includes("is stay finder safe")) {
          ...
          lowerCaseMessage.includes("your builder") ||
          lowerCaseMessage.includes("who is your maker") ||
          lowerCaseMessage.includes("who created stay finder") ||
          lowerCaseMessage.includes("who built stay finder") ||
          lowerCaseMessage.includes("who is the founder of stay finder")
        ) {
          aiResponse = "I was built by Jaspreet Singh.";
          matched = true;
          ...
          lowerCaseMessage.includes("romantic getaway") || 
          lowerCaseMessage.includes("romantic trip") ||
          lowerCaseMessage.includes("romantic place") ||
          lowerCaseMessage.includes("romantic stay") ||
          lowerCaseMessage.includes("romantic places") ||
          lowerCaseMessage.includes("romantic stays") ||
          (lowerCaseMessage.includes("romantic") && (lowerCaseMessage.includes("place") || lowerCaseMessage.includes("stay") || lowerCaseMessage.includes("most"))) ||
          lowerCaseMessage.includes("romatic getaway") ||
          lowerCaseMessage.includes("romatic trip") ||
          lowerCaseMessage.includes("romatic place") ||
          lowerCaseMessage.includes("romatic stay") ||
          lowerCaseMessage.includes("romatic places") ||
          lowerCaseMessage.includes("romatic stays") ||
          (lowerCaseMessage.includes("romatic") && (lowerCaseMessage.includes("place") || lowerCaseMessage.includes("stay") || lowerCaseMessage.includes("most")))
        ) {
        ...
        } else if (lowerCaseMessage.includes("best ratings") || lowerCaseMessage.includes("highest rated") || lowerCaseMessage.includes("highest-rated") || lowerCaseMessage.includes("highest rated stays") || lowerCaseMessage.includes("highest-rated stays")) {
        ...
        else if (lowerCaseMessage.includes("stay finder") || lowerCaseMessage.includes("stayfinder")) {
          aiResponse = "Stay Finder is a platform to find unique accommodations worldwide. We have verified hosts, secure payments, and a variety of listings from cabins to luxury villas. Please let me know what else you'd like to know!";
        } else {
        ...
        if (aiResponse !== "I'm sorry, I couldn't understand your request.") {
          matched = true;
        }
        ...
        if (!matched) {
          ...
          try {
            const cohereResponse = await getCohereResponse(updatedHistory);
            if (cohereResponse && cohereResponse.startsWith('Error:')) {
              aiResponse = "I'm sorry, I didn't understand your request. I can help you with any questions related to Stay Finder and its listings. Would you like to know more about specific accommodations, search for stays in a particular location, or understand the booking process? I can provide detailed information on amenities, prices, ratings, and unique features of various properties.";
            } else {
      ...
      <div style={panelStyle}>
        ...
        {messages.length === 0 && isContentSectionVisible ? (
          <div style={welcomeSectionStyle}>
            ...
            <button style={promptButtonStyle} onClick={() => handleSendMessage("Romantic Places For Couples")}>Romantic Places For Couples</button>
          </div>
        ) : (
                      ...
                      <div>
                        <div dangerouslySetInnerHTML={{ __html: (() => {
                          if (typeof msg.text === 'string') {
                            ...
                            return msg.text.message || msg.text.text || msg.text.content || JSON.stringify(msg.text);
                          } else {
        ...
        <div style={{ position: 'sticky', bottom: 0, left: 0, right: 0, background: '#1C1C1E', borderTop: '1px solid #333'}}>
          <div style={{ padding: '10px 15px' }}>
            ...
            <div style={{ paddingTop: 8, fontSize: '12px', color: '#888', textAlign: 'center' }}>
